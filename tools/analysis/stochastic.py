"""
 This file presents some popular stochastic processes implementations in Python used in quantitative finance.
...
class ModelParameters:
    """
    Stochastic model parameters holder
    """

    def __init__(self,
                 all_starting_asset_value, all_time, all_delta, all_sigma, gbm_mu,
                 jumps_lamda=0.0, jumps_sigma=0.0, jumps_mu=0.0,
                 cir_a=0.0, cir_mu=0.0, all_starting_rates=0.0, cir_rho=0.0,
                 ou_a=0.0, ou_mu=0.0,
                 heston_a=0.0, heston_mu=0.0, heston_vol0=0.0):
        # This is the starting asset value
        self.all_s0 = all_starting_asset_value
        ...
        self.all_r0 = all_starting_rates
...
def plot_stochastic_processes(processes, title='Stochastic Processes'):
    """
...
def simulate_cointegrating_series(d, n, mu, sigma, start_point_1, start_point_2):
    """
    Simulates two cointegrated time series
    
    :param d: correlation coefficient (becomes a random walk if d = 0)
    :param n: number of points to be simulated
    :param mu: mean of Gauss process for drawing increments
    :param sigma: std of Gauss process
    :param start_point_1: start point for first series
    :param start_point_2: start point for second series
    :return: ser1, ser2, ser1 - ser2
    """
    ser1 = np.zeros(n)
    ser2 = np.zeros(n)
    
    # These are the starting points of the random walk
    ser1[0] = start_point_1
    ser2[0] = start_point_2
    for t in range(1, n):
        # drunk and his dog cointegration equations
        ser1[t] = ser1[t - 1] + random.gauss(mu, sigma)
        ser2[t] = d * (ser1[t - 1] - ser2[t - 1]) + ser2[t - 1] + random.gauss(mu, sigma)
    return ser1, ser2, ser1 - ser2
